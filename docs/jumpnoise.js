...
function condense(_, ...args) {
    ...
    while (returnArgs.length >= 1 && returnArgs[0].match(defaultExprRE))
        returnArgs.shift();
    ...
    if (!hasParams)
        return hNV.value.replaceAll(/[KCVE]/g, replace);
    if (hEO.value != "" && propagate)
        return hEO.value.replaceAll(/[KCVE]/g, replace);
...
function applyHash() {
    ...
    for (i = 0; i < styles.length; i++) {
        let style = styles[i];
        if (parts[0] == style[1] && parts[1] == style[2] && parts[2] == style[3])
            break;
    }
...
function pickStyle(idx) {
    if (idx == styles.length)
        hCustomDetails.open = true;
