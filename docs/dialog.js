...
then send me an email to gukn4n4i at anonaddy.me.
...
[edit from 2023-09-07: this dialog subtree is outdated, i now include the content in the rss feed.]
...
const init = (_) => {
  ...
  ss.replace(`.cbutton { margin-top: 0.50em; }`)
  ...
  for (let part of data.split(/\n\n+/)) {
    ...
    if (part[0] == "@") {
      ...
      for (let opt of part.split("\n")) {
        ...
        opts += `<button class=cbutton data-target='${name}'>${text}</button><br>`
      }
      questions[id] += opts
      continue
    }
...
const render = (_) => {
  ...
  for (let elem of hquestion.getElementsByTagName("button")) {
    elem.onclick = (ev) => { location.hash = ev.target.dataset.target }
  }
}
